...
| `members` → `member_tokens` | 1:N | 회원이 여러 일회성 토큰 보유 (비밀번호 재설정 전용 — 이메일 인증 미구현) |
...
> 이 테이블은 **비밀번호 재설정 전용** 일회성 토큰만 저장합니다.  
> 이메일 인증은 구현하지 않습니다.
...
| `token_hash` | SHA-256 해시 (단방향) | 비밀번호 재설정 토큰 원문 미저장 |
...
| `member_tokens` | 만료 후 스케줄러 물리 삭제 | `PASSWORD_RESET`: 1시간 |
