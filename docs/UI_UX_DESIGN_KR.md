...
│  ├───────────────────────────────────────────────────────────┤  │
│  │  INPUT CONTROL  (하단 고정)                               │  │
│  │  [LLM ▼] [System ▼]  [□ Visualize] [□ Analyze Result]    │  │
│  │  ─────────────────────────────────────────────────────    │  │
│  │  textarea                                           [↑]   │  │
│  └───────────────────────────────────────────────────────────┘  │
│                                                                 │
...
| 항목 | 내용 |
|------|------|
| 위치 | 입력 컨트롤 상단, **시스템 드롭다운의 왼쪽** |
| 옵션 표시 라벨 | `Local`, `Anthropic (Cloud)` |
| 옵션 전송 값 | `ollama`, `anthropic` (요청 페이로드 `provider` 필드) |
| 기본값 | `GET /api/llm/providers`의 `defaultProvider` 응답에 따름 (보통 `Local`) |
| 데이터 | `GET /api/llm/providers` — 응답의 `availableProviders` 배열에 포함된 provider만 표시. 운영 환경에 `ANTHROPIC_API_KEY` 미설정이면 `anthropic`이 응답에서 제외되어 옵션 자동 숨김 |
| 동작 | 매 요청마다 현재 선택값을 페이로드에 포함하여 전송. 메시지마다 자유롭게 전환 가능 (세션 단위 고정 아님) |
| 시각적 힌트 | 라벨 옆에 작은 인디케이터 아이콘 (Local: 🖥 형태의 단색 SVG / Anthropic: ☁ 형태의 단색 SVG). 색은 `--color-text-secondary` 유지 |
| 비활성화 조건 | 처리 중(요청 진행 중), 히스토리 조회 모드 |

> **UI 라벨 ↔ API 값 매핑은 절대 하드코딩 금지.** `GET /api/llm/providers`가 `{provider: "ollama", displayName: "Local"}` 형태로 라벨까지 내려주도록 백엔드와 합의 (API_SPECIFICATION 갱신 필요).

> **A/B 비교 흐름.** 같은 시스템·같은 질문을 LLM만 바꿔 두 번 보내면 채팅 영역에 결과 두 건이 순차 표시됩니다. 메시지 메타에 `via Local` / `via Anthropic` 작은 캡션을 붙여 어떤 LLM이 생성했는지 식별할 수 있게 합니다.

### 6-3. 시스템 선택 드롭다운

| 항목 | 내용 |
|------|------|
| 위치 | 입력 컨트롤 상단, **LLM 드롭다운 오른쪽** |
